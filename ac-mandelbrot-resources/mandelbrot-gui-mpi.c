...
void keypress(unsigned char key, int x, int y)
{
	...
	case '.':
		GLOBAL_max_iter += 128;
		if (GLOBAL_max_iter > 1 << 15)
			GLOBAL_max_iter = 1 << 15;
		printf("max iter: %d\n", GLOBAL_max_iter);
		break;
	...
	case 'Z': // simulate many mouse clicks in order to dive fully in zoomin
		...
		{
			if (zoomin_x == GLOBAL_zoomin_num_pairs - 2)
				GLOBAL_refresh = 1;
...
void render()
{
	...
	glVertex2i(0, 0);
	glTexCoord2f(x, 0);
	glVertex2i(GLOBAL_width, 0);
	glTexCoord2f(x, y);
	glVertex2i(GLOBAL_width, GLOBAL_height);
	glTexCoord2f(0, y);
	glVertex2i(0, GLOBAL_height);
...
int main(int c, char **v)
{
	...
	{ // Outras tasks executam só o calc mandel
		for (int c = 0; c < 20; c++)
			calc_mandel();
	}
